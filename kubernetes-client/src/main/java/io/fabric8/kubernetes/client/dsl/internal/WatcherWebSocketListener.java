...
abstract class WatcherWebSocketListener<T> extends WebSocketListener {
    ...
    /**
     * True if an onOpen callback was received on the first connect attempt, ie. the
     * watch was successfully started.
     */
    ...
    protected WatcherWebSocketListener(AbstractWatchManager<T> manager, BlockingQueue<Object> queue,
            AtomicReference<WebSocket> webSocketRef) {
    ...
    public void onFailure(WebSocket webSocket, Throwable t, Response response) {
        ...
        if (response != null) {
            ...
            // We do not expect a 200 in response to the websocket connection. If it occurs,
            // we throw
            // an exception and try the watch via a persistent HTTP Get.
            // Newer Kubernetes might also return 503 Service Unavailable in case WebSockets
            // are not supported
